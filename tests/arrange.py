...
def select_trump(game_to_select: Game) -> None:
    ...
    game_to_select.act(
        SelectTrump(game_to_select.active_round.active_player.identifier, SelectableSuit.SPADES)
    )
...
def __get_bidding_game(seed: Optional[str], automate: bool = False) -> Game:
    ...
    new_game = Game(
        players=Group(
            list(map(lambda identifier: Player(str(identifier), automate=automate), range(4)))
        ),
